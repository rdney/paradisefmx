    ...
    def get_queryset(self):
        ...
        if month and year:
            try:
                ...
                qs = qs.filter(due_date__gte=first_day, due_date__lte=last_day)
...
def serve_attachment(request, pk, attachment_pk):
    """Serve attachments - redirects to Cloudinary or serves local files."""
    repair_request = get_object_or_404(RepairRequest, pk=pk)
    attachment = get_object_or_404(Attachment, pk=attachment_pk, repair_request=repair_request)
    ...
    if file_url.startswith('http'):
        # Production: redirect to Cloudinary URL
...
class CostOverviewView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for month in range(1, 13):
            ...
            # Aggregate costs for requests with due_date in this month
